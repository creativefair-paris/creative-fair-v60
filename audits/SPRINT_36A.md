...
0bis. **Robustesse parsing JSON Anthropic** (post-test visuel Lead) — `extractJsonFromText` strip les fences markdown (```` ```json ```` ou ```` ``` ````), puis fallback sur substring entre premier `{` et dernier `}`. Réponse brute Anthropic loguée en `console.error` quand `JSON.parse` échoue. Cascade modèles étendue à 3 niveaux : `claude-opus-4-5` → `claude-opus-4-1` → `claude-sonnet-4-5` (404 only, autres erreurs court-circuitent). Modèle effectivement utilisé tracé dans `programmes.context_generation.model` et logué côté serveur.
0ter. **Encodage écran d'attente** (post-test visuel Lead) — `L\u2019IA` était écrit en literal dans le JSX (texte enfant), donc rendu tel quel par React. Remplacé par le caractère typographique direct `’`. Les autres `\u2019` du fichier sont dans des string literals JS (`QUESTIONS` array) et restent valides.
