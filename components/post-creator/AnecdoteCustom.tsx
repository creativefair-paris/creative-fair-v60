...
const STEPS: Step[] = [
  { id: 1, title: 'Anecdote', loadingPhrase: 'Je relis ton anecdote…' },
...
export function AnecdoteCustom({ postId, draft, onDraftChange }: Props) {
  ...
  function startStep1() {
    const text = state.anecdoteText.trim()
    if (text.length < 10) {
      setState((s) => ({ ...s, error: 'Raconte ton anecdote en quelques phrases.' }))
      return
    }
    void runStep(1, text)
  }
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-1">
        ...
        >
          Tu nous racontes ton histoire. On construit la publication avec toi.
        </p>
      </div>
        ...
        {STEPS.map((step) => {
          ...
          return (
            ...
            >
              ...
              {step.id === 1 && isCurrent && state.status === 'pending' && !result && (
                <div className="space-y-2">
                  <textarea
                    value={state.anecdoteText}
                    onChange={(e) =>
                      setState((s) => ({ ...s, anecdoteText: e.target.value }))
                    }
                    rows={5}
                    placeholder="Raconte ton histoire en quelques phrases. Ce qui s'est passé, ce que tu as ressenti, pourquoi tu veux la partager."
